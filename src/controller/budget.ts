...
export default function useBudget(dataTask: any) {
  ...
  const sortMaxBudget = () => {
    if (typeof dataTask.sortData == 'undefined') {
      dataTask.task.sort((a: any, b: any) =>
        a.budget.value < b.budget.value
          ? 1
          : b.budget.value < a.budget.value
          ? -1
          : 0
      )
    } else {
      dataTask.sortData.sort((a: any, b: any) =>
        a.budget.value < b.budget.value
          ? 1
          : b.budget.value < a.budget.value
          ? -1
  ...
  const queryBudget = (budget: string, platform: any) => {
    checkQueryPage().then((respond) => {
      respond == true
        ? routePush(budget, platform, queryRouter.value.page)
        : routePush(budget, platform)
    })
  }
