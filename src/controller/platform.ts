...
export default function usePlatform(dataTask?: any) {
  ...
  const queryPlatform = (platform: any, budget: string) => {
    checkQueryPage().then((respond) => {
      respond == true
        ? routePush(platform, budget, queryRouter.value.page)
        : routePush(platform, budget)
    })
  }
  ...
  const sortPlatform = (platform: any, budget?: any) => {
    ...
    dataTask.task.forEach((element: any, index: number) => {
      const matchPlatform = element.platforms.filter(
        (data: string) => data.toLowerCase() == platform
      )
  ...
  const getPlatform = () => {
    const getDataPlatform = dataTask.task.map((element: any) =>
